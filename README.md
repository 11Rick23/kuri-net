...
プロフィールの冒頭には、所属・関心分野・連絡先をまとめた名刺を表示します。名刺はマウスのホバー位置に合わせて軽く傾き、余白を大きくドラッグすると裏側が少し見えるまで傾けられます。反射は静止時にも淡く表示し、操作に滑らかに追従します。マウスを外すかドラッグを終えると正面へ戻ります。スマホでは横方向のドラッグを使え、縦スクロールとピンチズームも維持します。「動きを減らす」設定時は回転を無効にします。見た目は `src/features/profile/components/ProfileBusinessCard.module.css`、操作は `TiltableCard.tsx` で調整できます。
