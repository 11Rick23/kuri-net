...
プロフィールの冒頭には、所属・連絡先をまとめた名刺を表示します。名刺はマウスのホバー位置に合わせて軽く傾き、余白をドラッグすると角度制限なく回転できます。反射は静止時にも淡く表示し、操作に滑らかに追従します。速くドラッグして離すと、その勢いで回転を続け、減速してから近い向きの表面へ戻ります。途中で掴み直すと回転を止めて操作できます。スマホでは横方向のドラッグを使え、縦スクロールとピンチズームも維持します。「動きを減らす」設定時は回転を無効にします。見た目は `src/features/profile/components/ProfileBusinessCard.module.css`、操作は `TiltableCard.tsx` で調整できます。
